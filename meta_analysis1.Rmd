...
Check the structure of the data frame and delete columns which are not necessary, for instance columns that report median. Next, check the structure of obtained data frame. 
...
Now lets have a look at the patient groups that represent a patient primary disease from each study. By looking at the histogram we can see how many observations we have for each group.
```{r}
ggplot(data_mean_rS) + geom_bar(aes(x = patient_g)) + coord_flip()
```
